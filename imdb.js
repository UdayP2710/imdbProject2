...
async function fetchData(search,check,url) {
    try {
      let data = await fetch(`https://www.omdbapi.com/?apikey=56566858&${url}=${search}`);
...
function suggestionBar(moviearr)
{
    ...
    movietitlearr.forEach((movie)=>{                            // for each movie title we are using eventlistener......
        let id = movie.querySelector(".movieid").textContent;
        movie.addEventListener("click", ()=>{                  // on clicking any list it redirect controls to fetch data.....
...
function redirectToMoviePage(obj){
    ...
    window.location.href = "http://127.0.0.1:5500/imdbmovie.html"; // redirecting to imdbmovie.html page......
}
...
function toFavMoviePage(favobj){
    ...
    window.location.href = "http://127.0.0.1:5500/favmovie.html";         // redirecting to favmovie.html page......
}
